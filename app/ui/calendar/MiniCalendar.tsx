...
const MiniCalendar = () => {
  ...
  useEffect(() => {
    // Atualiza o currentDate quando selectedDate muda
  ...
  return (
    <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      <header className="calendar-header flex justify-between items-center bg-blue-500 text-white p-4">
        ...
        <h2 className="text-xl font-semibold w-24 text-center"> {/* Defina uma largura fixa */}
          {capitalizeFirstLetter(currentDate.toLocaleString("default", { month: "long", year: "numeric" }))}
        </h2>
        <button
      ...
      <div className="grid grid-cols-7 gap-1 p-2">
        {days.map((day, index) => (
          <button
            ...
            className={`py-2 rounded-lg hover:bg-blue-100 transition duration-200 ${day.getMonth() !== currentDate.getMonth()
              ? "text-gray-400"
      ...
      {selectedDate && (
        <div className="p-4 border-t">
          ...
          <p className="mt-2 text-gray-600 truncate" style={{ maxWidth: '200px' }}> {/* Use truncate para evitar quebra de layout */}
